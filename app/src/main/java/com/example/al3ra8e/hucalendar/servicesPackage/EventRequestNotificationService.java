...
public class EventRequestNotificationService extends IntentService {

    public static boolean ServiceIsRun = false;
    public static long lastCreatedEvent = 0 ;
    public static long personID = 0 ;

    ArrayList<Event> events ;

    public EventRequestNotificationService() {
    ...
    protected void onHandleIntent(Intent workIntent) {
        ...
        events = new ArrayList<>() ;
            ...
            params.put("last_created",lastCreatedEvent+"") ;
            params.put("person_id",personID+"") ;

            String url = new UrlBuilder(AccessLinks.GET_ALL_NEW_REQUESTS).setUrlParameter(params).getUrl() ;
                new RequestBuilder()
                        .setUrl(url)
                                    ...
                                    if(response.has("Events")) {
                                        ...
                                        for (int i = 0; i < json.length(); i++) {
                                            ...
                                            Event event = new Event()
                                                    ...
                                                    .build();
                                            events.add(event);
                                            lastCreatedEvent = newDataItem.getLong("last_created");
                                        }
    ...
    public void sendBroadcastData(ArrayList<Event> events) {
        for(int i = 0 ; i < events.size() ; i++){
            ...
            intent.putExtra(Keys.SERVICES_STATE_KEY, Keys.EVENT_REQUEST_SERVICE);
